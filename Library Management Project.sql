create database library_databases;
use library_databases;
...
-- 2)How many copies of the book titled "The Lost Tribe" are owned by each library branch? tbc.book_copies_no_of_copies
SELECT 
    tbc.book_copies_no_of_copies, tlb.library_branch_branchname
FROM
    tbl_book tb
        JOIN
    tbl_book_copies tbc ON tb.book_bookid = tbc.book_copies_bookid
...
FROM
    ...
    tbl_book_loans tbl ON tb.borrower_cardno = tbl.book_loans_cardno
        LEFT JOIN
    tbl_book ON tbl_book.book_bookid = tbl.book_loans_bookid
WHERE
    tbl_book.book_title IS NULL;
...
FROM
    ...
    tbl_library_branch tlb ON tbl.book_loans_branchid = tlb.library_branch_branchid
        LEFT JOIN
    tbl_borrower tb ON tbl.book_loans_cardno = tb.borrower_cardno
        LEFT JOIN
...
SELECT 
    tlb.library_branch_branchname, COUNT(tb.book_title) AS freq
FROM
